...
\subsection{Closing the coverage gap without retraining}

The memory is used only at inference time for retrieval, so its coverage can be
raised without touching the model. We generated Type-1 descriptions (DeepSeek,
same style as the trained entries) for the \num{318} relations that appear in
the evaluation gold queries but were absent from the training-built memory --
\num{294} of them GrailQA zero-shot -- and re-ran reconstruction on the
\emph{unchanged} v14 plans. Table~\ref{tab:ext} shows the effect: zero-shot F1
more than quadruples while i.i.d.\ and compositional are unchanged, confirming
that the collapse was purely missing coverage.

\begin{table}[htbp]
  \centering
  \caption{GrailQA dev F1 with the memory extended to cover the evaluation
  relations. No retraining: only the retrieval memory changed.}
  \label{tab:ext}
  \begin{tabular}{lS[table-format=4.0]S[table-format=1.4]S[table-format=1.4]}
    \toprule
    Split & {n} & {v14} & {v14 + memory ext.} \\
    \midrule
    i.i.d.        & 1593 & 0.8185 & 0.8172 \\
    compositional & 1514 & 0.4266 & 0.4266 \\
    zero-shot     & 3656 & 0.0873 & \bfseries 0.3796 \\
    \midrule
    overall       & 6763 & 0.3355 & \bfseries 0.4932 \\
    \bottomrule
  \end{tabular}
\end{table}

This is the fix the paper's own limitation section proposes (``model the whole
Freebase into a memory''). One honesty caveat: the added relations were
enumerated from the dev gold queries, i.e.\ with knowledge of which relations
occur. The deployable form adds the entire KG schema instead, which would cover
the same relations without peeking but introduce more retrieval distractors, so
these figures are an upper bound on what the coverage fix alone can reach. The
result nonetheless isolates the cause: GrailQA zero-shot is a memory-coverage
problem, not a reasoning or operator failure, and it is addressable entirely at
inference time.
