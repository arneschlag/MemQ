...
transfer: it matches not a single gold relation multiset and its edge hitting
rate is below one per cent. Joint training lifts every measure substantially.

\begin{table}[htbp]
  \centering
  \caption{GrailQA, \num{200} questions (122 zero-shot, 45 i.i.d.,
  33 compositional). Same questions, same memory settings, same endpoint.}
  \label{tab:grailqa}
  \begin{tabular}{lS[table-format=1.4]S[table-format=1.4]S[table-format=2.1]}
    \toprule
    Metric & {v9} & {v14 (cp15000)} & {Factor} \\
    \midrule
    Hits@1              & 0.0400 & \bfseries 0.2850 & 7.1 \\
    Macro-F1            & 0.0365 & \bfseries 0.2826 & 7.7 \\
    Structure accuracy  & 0.0000 & \bfseries 0.2300 & {--} \\
    EHR                 & 0.0061 & \bfseries 0.2340 & 38.4 \\
    GoldGED $\downarrow$ & 3.5156 & \bfseries 1.9326 & {--} \\
    \bottomrule
  \end{tabular}
\end{table}

An important caveat: every question in this sample has
\texttt{function: none}. The v9-era evaluation adapter excluded count, extrema,
comparison and literal questions, so the operators added for v14 are
\emph{not} exercised here. The sample measures GrailQA generalisation
(61\,\% zero-shot relations), not the operator extension.
